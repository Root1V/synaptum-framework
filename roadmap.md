...
| SYN-28 | `HECHO` | Sistema de prompts | `PromptTemplate` versionado, proveedores encadenados por prioridad. **JSON sin dependencias**, YAML opcional — en la v0.4 el YAML arrastraba una dependencia para todos. Una variable sin valor **falla al renderizar** en vez de dejar la llave suelta en el prompt |
| SYN-29 | `HECHO` | Suite de tests del núcleo | **Íntegramente sobre `FakeModel`**, sin depender de inferencia real — obligatorio por P10, no preferible. Cierra la contradicción de la v0.4, que vendía testabilidad sin un solo test. **324 tests, y 269 pasan sin los contratos compartidos ni credencial ninguna**: comprobado quitando del entorno todo lo que pudiera dar acceso a inferencia |
...
| SYN-66 | `HECHO` | `ReplayGateway` sobre el corpus | Los cuerpos grabados pasan por el **adaptador real**, no por un atajo: al bucle llega lo que llegaría en producción. Recorre los caminos que nadie escribe a mano — `content` nulo con tool calls, argumentos como cadena, `usage` derivado — porque nadie escribe el caso que no se le ocurre |
...
| SYN-67 | `HECHO` | Transporte HTTP de desarrollo | Cierra el hueco entre un adaptador puro y un endpoint real: sin esto no había camino de `pip install` a un agente hablando con un modelo. Solo stdlib — `urllib` en un hilo, porque el núcleo no tiene dependencias y no las va a tener por esto. **De desarrollo**: en el camino gobernado la llamada no ocurre en este proceso |
| SYN-68 | `HECHO` | `Agent.stream()` | El streaming existía en las cuatro costuras y el bucle no lo usaba, así que nadie podía ver un token llegar. Va aparte de `run` porque **cambia el tipo de lo que se cede**. `run` y `stream` dejan de ser `async def`: devuelven el iterador en vez de envolverlo, porque un envoltorio se lleva el `GeneratorExit` y deja abierto el de dentro — la cancelación llegaba con el recolector |
| SYN-69 | `HECHO` | Ejemplos ejecutables | Cuatro, corriendo sin inferencia por defecto y contra un modelo real con dos variables de entorno, **sin tocar el fichero**. El 02 no afirma la reanudación: cuenta llamadas y la mide |
| SYN-70 | `HECHO` | Independencia verificada por test | El paquete no nombra ningún despliegue concreto, no carga ningún puente de proveedor sin pedirlo, no lleva ni una ruta absoluta, y la suite pasa entera sin los contratos compartidos. Se degradaba por descuido —una ruta escrita al depurar, una docstring que cuenta el despliegue de quien la escribió como si fuera el diseño— y eso no duele en la máquina donde se escribió, que es el problema |
| SYN-71 | `HECHO` | Identidad de paso como clave de idempotencia | `(run_id, step_id)` es determinista por construcción, así que reanudar produce la misma clave sin coordinar nada. Cierra el hueco que el journal solo no tapa: morir **después** de mandar la petición y **antes** de registrar el resultado hacía que el replay reintentara y pagara una segunda generación. Verificado contra la plataforma: segunda llamada con la misma identidad → `idempotent_replay=True`, mismo texto, sin generar |
| SYN-72 | `HECHO` | Un replay no se suma al total del run | Su `usage` describe la generación original, que ya se contó. El paso lo registra tal cual —el journal cuenta lo que ocurrió— y el total cuenta lo que se pagó. No fallaba ni avisaba: solo inflaba la factura |
...
| SYN-38 | `HECHO` | Métricas de economía de contexto | Acierto de caché ponderado por tamaño, reescrituras de prefijo con **qué** cambió, y crecimiento de entrada por turno. Se calcula **del journal**, así que vale sobre un run terminado y sin el agente delante. **Solo calcula: no exporta** — el formato lo fija `SYN-37` con la plataforma que lo consuma, y emitir uno propio ahora sería construir para tirar |
...
| SYN-40 | `HECHO` | Carga diferida de tools | El catálogo se mueve **al historial**, no al prefijo: dos herramientas fijas —buscar y usar— y el esquema llega como resultado. Cargar bajo demanda habría sido peor, y está medido: añadir **una** herramienta a mitad de run tira `cache_read` de 1.443 a **0**, historial incluido. 3.737 → 142 tokens de prefijo con 40 herramientas. El despachador hereda el mayor riesgo del catálogo, o esconderlas detrás de una las blanquearía |
...
| SYN-44 | `EN CURSO` | Adaptador A2A | Cliente A2A (binding HTTP+JSON) y `RemoteDelegate` con el mismo contrato que uno local: **el bucle no distingue** si el subagente está en el proceso o al otro lado de una red. Reanudar es una **consulta** —`ListTasks(contextId)` con `contextId` = nuestro `sub_run_id`— y no una apuesta sobre si el servidor deduplica. Solo stdlib: el SDK oficial arrastra once dependencias. Falta el lado servidor —con la regla de que **un agente gobernado no debe tener puerta propia**, que es el mismo agujero del revés— y el proxy de gobierno, que es de Aeon (`A2A-002`). **Almacén compartido confirmado**: uno por despliegue, con `PRIMARY KEY (run_id, step_id, phase)` sin componente de contenedor, así que un paso remoto interrumpido lo reencuentra quien reanude |
...
| SYN-48 | `HECHO` | Extra `[axonium]` devuelto · Axonium `AXO-23` | `axonium 1.0.0rc1` está en PyPI. Fijado al rc a propósito: sin versión estable, un `>=1` no resuelve |
...
| SYN-52 | `HECHO` | Puente Synaptum → SDK de plataforma | **No es un `Provider`**: Axonium ya normaliza, así que esto traduce entre dos vocabularios normalizados. Pasarlo por el lector de cable perdería `cache_read` y `estimated` en silencio. Encaja como modelo de `LocalGateway` |
...
| SYN-74 | `HECHO` | **Publicar en un índice** | `1.0.0rc1` **publicado y verificado en TestPyPI**: instalado desde el índice en un entorno limpio, con salida tipada, `py.typed` y cero dependencias. Automatizado con **Trusted Publishing (OIDC)** — no hay token en ninguna parte, así que el secreto no está mejor guardado: no existe. Publicado también en **PyPI**: `pip install synaptum` funciona desde cualquier sitio |
| SYN-75 | `HECHO` | Plantilla de proyecto de agente | `plantilla/` — proyecto completo: herramientas con riesgo declarado, salida tipada, la puerta que elige doble o modelo real, y cuatro tests que enseñan a probar un agente. **Con tres tests en el repo que la ejecutan de verdad**, contra el código local y no contra el publicado, para que un cambio que la rompa falle antes de publicar. La primera versión de esos tests no cazaba nada —uv servía un wheel cacheado y no veía el fuente—, así que hace falta `--with-editable` |
| SYN-76 | `HECHO` | Guía de adopción | Ocho páginas en `docs/`, **una sola fuente y dos salidas**: Markdown para agentes y para quien lee el repositorio, HTML generado para personas. Un test falla si divergen, otro comprueba que no nombren símbolos que no existen, y otro que lo que dan por pendiente siga pendiente en este roadmap |
| SYN-79 | `HECHO` | Referencia generada del código | Los 98 símbolos públicos con firma, campos, métodos y docstring, introspeccionados del paquete. Escribirla a mano sería escribir algo que caduca, y una referencia caducada manda a buscar un símbolo que ya no está. Dos tests: que corresponda al código, y que **ningún exportado quede sin documentar** |
| SYN-78 | `HECHO` | Documentación publicada | GitHub Pages desde el Markdown, resaltado de sintaxis en tiempo de construcción —sin JavaScript ni CDN, se lee sin red— y enlazada desde PyPI como `Documentation`. El enlace aparece en PyPI con **la siguiente versión publicada**: los metadatos de una versión ya subida no se pueden cambiar |
